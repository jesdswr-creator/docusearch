...
Task ID: v1.7.6-pdf-duplicates-ocr-theme
Agent: arena
Task: User feedback on v1.7.5 - AI search now acceptable ("somewhat good... but ok" - left as-is, additive fusion kept); splash colors must match the current button color; PDF preview still "error opening" (was working earlier); non-vertical (landscape/sideways) PDFs OCR'd incorrectly (rotation); duplicates check still returns a single file whose twin was already deleted.

Work Log:
- PDF PREVIEW (ROOT CAUSE FOUND - real broken code): PdfiumDocument::loadFromFile read the PDF into a LOCAL QByteArray and passed it to FPDF_LoadMemDocument. PDFium's public contract (fpdfview.h) says "the memory buffer must remain valid when the document is open" - the buffer is wrapped in a lazy stream and read during every later render/text call. The local died at return, so every page operation after load was a USE-AFTER-FREE: intermittent "error opening PDF", garbled pages, garbage OCR/extraction text. Verified against the pdfium source (CFX read-only span stream + the documented contract). Fix: the bytes now live in a QByteArray MEMBER (m_fileBytes) whose lifetime matches the document; cleared only after FPDF_CloseDocument. Locked-file retry extended to 3x150ms. This is the "broken code" the user suspected - it also explains why the failure was flaky ("was there earlier") and got worse once the v1.7.2 OCR pool started rendering PDFs on background threads (more heap churn, more buffer reuse).
- OCR ROTATION: verified in the pdfium source that FPDF_RenderPageBitmap + FPDF_GetPageWidth/Height ALREADY apply the page's /Rotate (CPDF_Page::UpdateDimensions folds rotation into page_matrix_ and swaps page size) - so /Rotate pages were never the problem. The real case is scans stored SIDEWAYS inside the page (rotated bitmap, no /Rotate entry): PDFium faithfully renders them sideways and Windows OCR reads vertical text as garbage. Fix: auto-orientation in OcrWorkerPool - OCR the upright raster first; if it yields < 3 words, retry 90/180/270 degrees using PDFium's native rotate parameter (renderPage gained an optional rotate arg that swaps the raster dims for quarter turns) and keep the orientation that reads the most words. Extra OCR runs only happen when the upright pass is nearly empty.
- DUPLICATES SINGLE FILE: two more holes closed. (1) The results pane kept showing the PREVIOUS check's list whenever the new check found nothing - the user deleted the twin, re-ran the check, the message box said "no duplicates" but the stale list still showed the lone survivor. The pane is now cleared (setResults({})) on the empty path. (2) NEW stale-hash gate: a stored hash only proves a file equals its partner if the content is unchanged - if size or mtime moved since the scan (2 s FAT tolerance), the row is dropped from the candidate set (the next scan refreshes the hash). This kills false pairs where a different file later appeared under the deleted duplicate's name.
- SPLASH THEME MATCH: the splash used a hardcoded navy palette that clashed with the app's theme. SplashOverlay now takes a ThemeColors set (card gradient, text, caption, slot, shadow) and main.cpp derives them from the SAME palette tokens as MainWindow::applyTheme() - the progress chunk + magnifier render in the exact button color (@primary@: #4d8df6 Midnight / #2563eb Daylight), the card follows the theme surfaces. The saved darkMode is read before the splash is shown.
- THEME WIRING BUG (audit find): applyTheme() switches on pastelTheme_ which was NEVER synced from the persisted darkMode setting - the saved dark theme was IGNORED at startup (default pastelTheme_=0 = Light) and the theme toggle flipped pastelTheme_ without persisting darkMode, so the chosen theme was lost on every restart. All four sites (load, toggle, Settings-applied x2) now keep settings_.darkMode <-> darkMode_ <-> pastelTheme_ in sync and the toggle persists.
- Version 1.7.6 (CMakeLists + Constants.h kAppVersion + changelog).
- Verified: every changed TU passes -fsyntax-only against a Qt 6.10.3 header farm on this sandbox (PdfiumDocument, OcrWorkerPool, MainWindow, main.cpp, PdfPreview, PreviewPane, PdfExtractor); brace/paren balance unchanged by the edits (pre-existing single stray paren in a string literal untouched). Windows CI is the runtime gate.

---
Task ID: v1.7.7-allowlist-splash-duplicates
Agent: main
Task: User feedback (against the v1.7.6 line) - splash animation still glitchy; .md files must not be scanned or shown ("only doc,xls,ppt,pdf, jpg,png etc."); duplicates still list files whose second copy was moved or deleted. Rebases the independent 1.7.5 work from the parallel session onto the remote v1.7.6 (arena-ai-coding-agent) as one merged v1.7.7.

Work Log:
- REBASE CONTEXT: remote main moved to 620f654 ("v1.7.6" by arena-ai-coding-agent[bot]) while the local session had an independent v1.7.5 commit (cc39db3). Surveyed the remote commit first: it KEPT the v1.7.4 bounce animation (sin reversal + alpha dip), KEPT the all-extension hash grouping in the duplicates SQL, added ThemeColors theming to the splash, a stale-hash gate + canonical-path dedupe + setResults({}) to duplicates, and did NOT add any extension allowlist (kDocumentExtensions still listed md/txt/rtf/csv). Merged release is versioned 1.7.7 to supersede both lines; the remote's fixes are preserved intact.
- SPLASH GLITCH (root cause): two compounding defects. (1) The v1.7.4 bar BOUNCED left-right (sin-driven reversal) with an alpha dipping to 0.25 at each end - the chunk visibly stuttered, dimmed and reversed every ~1 s, which reads as a glitch rather than progress. (2) pumpSplash() ran ONE processEvents(30 ms) per milestone, so the 16 ms animation timer coalesced into at most one frame per milestone - the animation advanced in irregular jump-cuts (freeze -> jump -> freeze) across the 1-3 s constructor.
- Splash rework (merged with the v1.7.6 theme system): the bar is now a MATERIAL-STYLE SWEEP - two chunks chase each other left-to-right (each owns 60% of a 2400 ms cycle, offset 40%, easeInOutSine, fade in/out over the first/last 12% of its own window). No reversal, no mid-bar alpha dip. Status captions CROSSFADE (previous line fades out while the next fades in over 300 ms). All colors still come from ThemeColors/setThemeColors (main.cpp wiring untouched). M_PI replaced with a local kPi constant (MSVC _USE_MATH_DEFINES hazard), <algorithm> added for std::clamp, qsizetype narrowing made explicit. Animation math validated by simulation (scripts/splash_sim_v175.py): position always within slot, alpha always in [0,1], max contiguous motion 0.375 px/ms, no NaN, both-chunk overlap 479/2400 frames, no empty frames.
- Pumping rework: pumpSplash() now runs three 16 ms paced turns (2-3 frames per milestone, ~100 ms total extra startup), plus two new milestones (after the OCR probe + event-filter install, after geometry restore + refreshSavedSearches).
- MD / NON-DOCUMENTS (root cause): FOUR ingest paths disagreed. onFileAdded (FileWatcher) had a private allowlist that still included md/txt/csv/rtf - a Markdown note saved into an indexed folder was indexed AND text-extracted immediately. The hourly reconciliation walk and MetadataIndexer::scan had NO extension filter at all - they indexed every walked file (exe, zip, everything; kIgnoredExtensions was never referenced anywhere). Only scanFolderFast (Add Folder) filtered.
- Central allowlist: Constants::kIndexableExtensions (pdf/doc/docx/xls/xlsx/xlsm/ppt/pptx + jpg/jpeg/png/tif/tiff/bmp/gif/webp) + isIndexableExtension() is now the ONE gate; kDocumentExtensions trimmed to the same document set (keeps ocr_status logic consistent). All four ingest paths consult it: the hourly walk gates BEFORE inserting into `seen` (so the Pass-2 prune treats old md/exe/zip rows as unseen and deletes them - self-healing), the watcher gate replaced its private list, scanFolderFast replaced its private set, MetadataIndexer::scan gained the gate.
- One-time startup purge: purgeNonIndexableRows() (constructor, right after Schema::migrate) deletes every Files row whose lower(trim(extension)) is not in the allowlist (NULL extensions purged too) plus SearchIndex/BgeEmbeddings/EmbeddingChunks rows, transactional; count logged and surfaced via a 10 s status message. Unconditional by design - the gates guarantee these rows can never be re-added, so even rows on a currently-offline drive lose nothing that would come back.
- DUPLICATES (merged): remote's stale-hash gate (size/mtime re-check inside the walk) and canonicalFilePath identity dedupe and setResults({}) on the empty path are preserved; on top of them: (1) the inner "hash shared by >1" subquery now counts DOCUMENT rows only (two DISTINCT SQL markers %1/%2 keep substitution unambiguous) - a same-bytes image/archive can no longer keep a document's group alive after its document partner is gone; (2) the doc-type whitelist is derived from Constants::kDocumentExtensions so it can never drift; (3) display-time re-verification: processEvents() inside the walk lets files be moved/deleted MID-WALK after they passed the exists-check - the verified set is re-grouped with the survivors-only rule right before display, so a file whose partner vanished can never render as a pair.
- Docs synced to policy: HELP.md + README.md supported-file tables, installer/README.txt "WHAT GETS INDEXED" now list documents + images only and state that md/txt/csv/rtf/log and binaries are intentionally not indexed.
- Gates (scripts/gate_v175.py, updated for 1.7.7): brace/paren balance, version sync (Constants.h == CMakeLists == WiX fallback == 1.7.7), allowlist sanity (contains all user-requested types, excludes md/txt/csv/rtf/log), gate coverage on all four ingest paths, no private supportedExts still allowing md, splash sanity (no sin-bounce, no M_PI, crossfade present, ThemeColors intact). File-mode drift normalized to 644 pre-commit.
