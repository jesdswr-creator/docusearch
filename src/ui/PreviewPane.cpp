...
PreviewPane::PreviewPane(QWidget* parent) : QWidget(parent) {
    ...
    // Extracted text — the main content area (no thumbnail box)
    ...
    // thumbLabel_ not used — create dummy to avoid null
...
void PreviewPane::setThumbnail(const QPixmap& pix) {
    Q_UNUSED(pix);
    // Thumbnails disabled in this build
}
...
void PreviewPane::onOpenClicked() {
    if (!pathLabel_->text().isEmpty() && pathLabel_->text() != "Select a file to preview")
